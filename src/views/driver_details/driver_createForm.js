  ...
  const submitForm = async () => {
      ...
      try {
        const resp = await makeApiCall(
          `${BASE_URL}${PATH_DRIVER}`,
          'POST',
          JSON.stringify({
            ...otherData,
                DrivingLicenseNumber: parseInt(DrivingLicenseNumber),
                ExperienceInYears: parseInt(ExperienceInYears),
          })
        );

        if (resp.ok) {
          snackbar.enqueueSnackbar('Successfully created Driver', {
            variant: 'success',
          });
